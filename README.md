...
page-sitemap.xml       16 pages, 92 images
...
**The Instagram feed is excluded.** The home page carries 158 harvested tiles
against 19 images of its own — listing them would make the sitemap 89% someone
else's photographs, which are not this site's content in any sense Google cares
about. So would the Smash Balloon UI sprite, which is dropped by the same rule
that keeps everything on the image host.

Header and footer chrome needs no rule: the logo and the skyline background live
in `_header.neutral.html` and `_footer.html`, separate partials this never reads.
Checked rather than assumed — no image appears on more than a handful of the 19
content files.
...
no sitemap; a page listed while its own markup says `noindex`; a URL in two
sitemaps at once; a sitemap built but left out of the index, or listed and not
built; an empty sitemap; a missing or unparseable `lastmod`; an image off the
image host or from the Instagram feed; and `robots.txt` not pointing at the
index. Verified by planting four of those, each of which the check named:
...
an Instagram tile in the image list       …/instagram/abc.jpg is an Instagram tile
...
63 distinct images they list resolve on the image host.

`/blog-post-title/` and `/author/admin/` are listed because both say
`index, follow` on the live site and the port keeps parity. Whether they *should*
be is [the client's call](#three-things-left-for-the-client-to-decide), points 1
and 2 — and if the answer is `noindex`, the sitemaps follow them out on their
own: that rule is already in the generator and in the check.
